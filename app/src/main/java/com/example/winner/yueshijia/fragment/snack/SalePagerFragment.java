 ...
 * @desc ${TODD}
 */
public class SalePagerFragment extends BaseFragment<snacksBean> {
...
/*    @Nullable
    ...
    private XListView sale_pager_lv;
    ...
    public void handlerMessage(int action, snacksBean sBean, boolean isFirst) {
        ...
        if (sBean.getCode()==200){
            ...
            if (isFirst){
                ...
                sale_pager_lv.setAdapter(adpter);
                sale_pager_lv.stopRefresh();
            }
            else {
                datas.addAll(goods_list);
                sale_pager_lv.stopLoadMore();
            }
            sale_pager_lv.setPullLoadEnable(sBean.isHasmore()?true:false);
        }
    }

    @Override
    protected View initFragmentView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_sales_pager,container,false);
        View head = LayoutInflater.from(getActivity()).inflate(R.layout.item_goodlist, null);
        this.sale_pager_lv = (XListView)view.findViewById(R.id.sale_pager_lv);
        sale_pager_lv.addFooterView(head);
      //  sale_pager_lv.addHeaderView(head);
        return view;
    ...
    protected void initEvent() {
        sale_pager_lv.setXListViewListener(new XListView.IXListViewListener() {
            @Override
