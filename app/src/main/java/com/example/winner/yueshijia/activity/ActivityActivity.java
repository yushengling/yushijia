...
public class ActivityActivity extends BaseActivity<ActivityDetailBean> implements View.OnClickListener {
    ...
    protected void initData() {
        String topid = getIntent().getStringExtra(IntentConst.ACTIVITY_WEB);
        if (topid == null) {
            finish();
            return;
        }
        mControl.doGet(ACTIVITY_DETAIL_ACTION, 0, true, topid);
        Toast.makeText(getApplicationContext(), "" + topid, Toast.LENGTH_SHORT).show();

    }

    @Override
    public void handlerMessage(int action, ActivityDetailBean bean, boolean isFirst) {
        switch (action) {
            case ACTIVITY_DETAIL_ACTION:
                if (bean.getCode() == 200) {
                    ActivityDetailBean.DatasBean datasBean = bean.getDatas();
                    String content = datasBean.getGoods_content();
