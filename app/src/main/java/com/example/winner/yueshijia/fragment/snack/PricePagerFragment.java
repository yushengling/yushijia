 ...
 * @desc ${TODD}
 */
public class PricePagerFragment extends BaseFragment<snacksBean> {
...
/*    @Nullable
    ...
    private XListView price_pager_lv;
    ...
    public void handlerMessage(int action, snacksBean sBean, boolean isFirst) {
        ...
        if (sBean.getCode()==200){
            ...
            if (isFirst){
                ...
                price_pager_lv.setAdapter(adpter);
                price_pager_lv.stopRefresh();
            }
            else {
                datas.addAll(goods_list);
                price_pager_lv.stopLoadMore();
            }
            price_pager_lv.setPullLoadEnable(sBean.isHasmore()?true:false);
        }
    }

    @Override
    protected View initFragmentView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_price_pager,container,false);
        View head = LayoutInflater.from(getActivity()).inflate(R.layout.item_goodlist, null);
        this.price_pager_lv = (XListView)view.findViewById(R.id.price_pager_lv);
        price_pager_lv.addFooterView(head);
        //price_pager_lv.addHeaderView(head);
        return view;
    ...
    protected void initEvent() {
        price_pager_lv.setXListViewListener(new XListView.IXListViewListener() {
            @Override
